...
import (
	...
	"os/signal"
	"syscall"
	telegram_bot "vpn-tg-bot/internal/service/bot/telegram"
	...
	settings := telegram_bot.Settings{
		Settings: telebot.Settings{
			...
			OnError: func(err error, c telebot.Context) {
				fmt.Printf("[ERR]: Telegram bot: Telebot Handler: %v\n", err)
			},
			Verbose: true,
		},
	}

	bot, err := telegram_bot.New(settings, storage, subsService, vpnSeverManager)
	if err != nil {
