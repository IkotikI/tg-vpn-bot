...
Array.from(searchable_dropdowns).forEach((group) => {
    SearchableDropdown(group as HTMLElement)
});

function SearchableDropdown(group: HTMLElement) {
    const dropdownInput = group.getElementsByClassName('dropdown-input')[0] as HTMLInputElement;
    ...
    const searchInput = group.getElementsByClassName('search-input')[0] as HTMLInputElement;
